module gas_exchange
  ...
  !Interfaces
  private 
  public content_from_po2,initial_gasexchange,set_perfusion_gradient,steadystate_gasexchange
  
  real(dp),private :: ideal_mass
...
!!! ######################################################################
  ...
  subroutine initial_gasexchange(initial_concentration, &
       inlet_concentration,p_ven_o2,shunt_fraction,cardiac_output, &
       surface_area,V_cap,species)
...
!!! ######################################################################
  ...
  subroutine set_perfusion_gradient(Gdirn,cardiac_output,COV,Qmax,Qmin)
    !*set_perfusion_gradient*: initialises a linear gradient in perfusion
    ! with iso-gravitational heterogeneity defined by the COV
    !DEC$ ATTRIBUTES DLLEXPORT,ALIAS:"SO_SET_PERFUSION_GRADIENT" :: SET_PERFUSION_GRADIENT
    
    integer,intent(in) :: Gdirn
    real(dp),intent(in) :: COV,cardiac_output,Qmax,Qmin
    ! Local variables
    integer :: ne,np2,nunit,xyz_index
    real(dp) ::  maxflow,max_z,min_z,Qmean,rando,range_z,Xi,xyz_multiply
    real(dp),allocatable :: random_list(:)
    character(len=60) :: sub_name

    ! --------------------------------------------------------------------------
    
    sub_name = 'set_perfusion_gradient'
    call enter_exit(sub_name,1)
...
!!! allocate memory for the gasex_field array, if not already allocated
    ...
    if(abs(Qmax-Qmin) < 1.0e-6_dp)then ! all the same
       gasex_field(ng_Qdot,1:num_units) = Qmax
    else        
!!! for each unit find the maximum and minimum coordinates in the Gdirn direction
       max_z = -1.0e+6_dp
       min_z = 1.0e+6_dp
...
!!! for each unit allocate a flow based on a gradient in the Gdirn direction, and
!!! perturb by a user-defined COV. This should be calling a random number generator.
       ...
       do nunit = 1,num_units
          ...
          Qmean = Qmax*Xi+Qmin*(1.0_dp-Xi)
          gasex_field(ng_Qdot,nunit) = max(0.1e-6_dp*Qmean,Qmean*(1.0_dp + rando*COV))
       enddo !nunit
    ...
    do nunit = 1,num_units
       ne = units(nunit)
       elem_field(ne_Qdot,ne) = gasex_field(ng_Qdot,nunit)
       unit_field(nu_perf,nunit) = elem_field(ne_Qdot,ne)
    enddo

!!! the following SHOULD take into account the smaller flow at units that subtend symmetric
!!! paths. i.e. that have higher number of effective units.
!!! sum the fields up the tree, and normalise
    call sum_elem_field_from_periphery(ne_Qdot) !sum the blood flows recursively UP the tree
    maxflow = elem_field(ne_Qdot,1)
    gasex_field(ng_Qdot,1:num_units) = cardiac_output*gasex_field(ng_Qdot,1:num_units)/maxflow
    unit_field(nu_perf,1:num_units) = cardiac_output*unit_field(nu_perf,1:num_units)/maxflow
    elem_field(ne_Qdot,1:num_elems) = cardiac_output*elem_field(ne_Qdot,1:num_elems)/maxflow

    call enter_exit(sub_name,2)

  end subroutine set_perfusion_gradient
  
!!!#############################################################################

  subroutine steadystate_gasexchange(cardiac_output,Vdot_alv,c_art_o2,c_ven_o2, &
       p_art_co2,p_art_o2, &
       p_i_o2,p_ven_co2,p_ven_o2,shunt_fraction,VCO2,VO2)
    !*steadystate_gasexchange*: calculates steady PO2 and PCO2 for a given
    ! V/Q to a unit. Whole lung values calculated by flow weighted sums.
...
!!! Calculate steady state gas exchange for CO2
    ...
    do while(continue)
       ...
       do nunit = 1,num_units  ! for each elastic/gas exchange unit
          ...
          ! the following converts normalised to absolute V and Q, V/Q
!          v_q = Vdot_alv/(cardiac_output*(1.0_dp-shunt_fraction)) * &
!               (unit_field(nu_Vdot0,nunit)/elem_field(ne_Vdot,1)) & 
          !               /unit_field(nu_perf,nunit)             ! the unit v/q
!          v_q = unit_field(nu_Vdot0,nunit)/unit_field(nu_perf,nunit)
          v_q = (unit_field(nu_Vdot0,nunit)*Vdot_alv/elem_field(ne_Vdot,1))/ &
               (unit_field(nu_perf,nunit)*(cardiac_output*(1.0_dp-shunt_fraction))/elem_field(ne_Qdot,1))
...
!          if(nunit.eq.1)then
!             write(*,*) 'unit 1:',v_q,p_cap_co2,unit_field(nu_Vdot0,nunit),unit_field(nu_perf,nunit)
!          endif
...
!!! calculate the content of CO2 in capillary blood, given the partial pressure
          c_cap_co2 = m*p_cap_co2/(1.0_dp + m*p_cap_co2)
...
!!! include the shunt fraction in total arterial CO2
       c_ven_co2 = m * p_ven_co2/(1.0_dp + m*p_ven_co2)
       c_art_co2 = c_art_co2*(1.0_dp-SHUNT_FRACTION)+c_ven_co2*SHUNT_FRACTION
       !!  summed alveolar pco2
       p_alv_co2 = p_alv_co2/elem_field(ne_Vdot,1)
       
!!! calculate the partial pressure of pulmonary arterial CO2:
       p_art_co2 = 1.0_dp/(m*(1.0_dp-c_art_co2)) ! initialise p_art_co2
       K = 0 !counter
       fun_co2 = m*p_art_co2/(1.0_dp+m*p_art_co2)-c_art_co2
       do while (abs(fun_co2).ge.1.0e-4_dp.and.(k.lt.200))
          K = K+1
          fdash = m/(1.0_dp+m*p_art_co2)**2
          p_art_co2 = p_art_co2 - fun_co2/fdash
          fun_co2 = m*p_art_co2/(1.0_dp+m*p_art_co2)-c_art_co2
       enddo !while
!!! find the p_ven_co2 for the new (target) content of venous CO2
       target_c_ven_co2 = c_art_co2 + VCO2/elem_field(ne_Qdot,1)
       !mL(CO2)/mL(blood)   mL/mL  [mm^3/s]/[mm^3/s]
       p_ven_co2 = 1.0_dp/(m*(1.0_dp-target_c_ven_co2))
       K = 0
       fun_co2 = m*p_ven_co2/(1.0_dp+m*p_ven_co2)-target_c_ven_CO2
       do while (abs(fun_co2).ge.1.0e-4_dp.and.(k.lt.200))
          K = K+1
          fdash = m/(1.0_dp+m*p_ven_co2)**2
...
!!! check whether p_ven_co2 and p_art_co2 have converged
    ...
    write(*,'('' For total blood flow ='',F6.2,'' L/min,&
         & alveolar ventilation ='',F6.2,'' L/min:'')') &
...
!               /unit_field(nu_perf,nunit)             ! the unit v/q
          v_q = (unit_field(nu_Vdot0,nunit)*Vdot_alv/elem_field(ne_Vdot,1))/ &
               (unit_field(nu_perf,nunit)*(cardiac_output*(1.0_dp-shunt_fraction))/elem_field(ne_Qdot,1))
!          v_q = Vdot_alv/(cardiac_output*(1.0_dp-shunt_fraction)) * &
!               (unit_field(nu_Vdot0,nunit)/elem_field(ne_Vdot,1)) & !made this change for using non-norm V
!               /unit_field(nu_perf,nunit)             ! the unit v/q
...
!!! find the p_ven_o2 for the new (target) content of venous O2
       target_c_ven_o2 = c_art_o2 - VO2/elem_field(ne_Qdot,1)
...
!!! check whether p_ven_o2 and p_art_o2 have converged
    ...
    do nunit = 1,num_units
       ne = units(nunit)
       np = elem_nodes(2,ne)
       node_field(nj_conc1,np) = p_cap_o2/(o2molvol*press_atm)
       node_field(nj_conc2,np) = p_cap_co2/(o2molvol*press_atm)
...
!!! this information is used in 'track_back' during expiration
    unit_field(nu_conc1,1:num_units) = gasex_field(ng_p_alv_o2,1:num_units)/&
         (o2molvol*press_atm) ! from mmHg to mmol/mm^3
    unit_field(nu_conc2,1:num_units) = gasex_field(ng_p_alv_co2,1:num_units)/&
         (o2molvol*press_atm) ! from mmHg to mmol/mm^3
...
!!! Calculate Hb-O2 saturation
       X=PO2*10.0_dp**(0.024_dp*(37.0_dp-temperature)+0.4_dp*(pH-7.4_dp)+ &
            0.06_dp*(log10(dble(40.0_dp))-log10(dble(PCO2))))
