...
!!!##########################################################################
  ...
  subroutine export_data_field_c(EXDATAFILE, filename_len, name, name_len, offset) bind(C, name="export_data_field_c")

    use iso_c_binding, only: c_ptr
    use utils_c, only: strncpy
    use other_consts, only: MAX_STRING_LEN, MAX_FILENAME_LEN
    use exports, only: export_data_field
    implicit none
    ...
    call export_data_field(filename_f, name_f, offset)

  end subroutine export_data_field_c
