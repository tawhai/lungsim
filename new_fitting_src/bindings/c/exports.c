...
void export_data_field_c(const char *EXDATAFILE, int *EXDATAFILE_LEN,
			    const char *name, int *name_len, int *offset);
...
void export_data_field(const char *EXDATAFILE, const char *name, int offset)
{
  int filename_len = strlen(EXDATAFILE);
  int name_len = strlen(name);

  export_data_field_c(EXDATAFILE, &filename_len, name, &name_len, &offset);
}
