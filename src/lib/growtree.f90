...
!!! terminal and remove a single seed point. Also involves modifying the local list of parents.
  ...
  subroutine grow_tree(surface_elems,parent_ne,angle_max,angle_min,&
    ...
    use mesh_utilities,only: calc_branch_direction,distance_between_points, &
         element_connectivity_1d,evaluate_ordering,get_local_elem_2d, &
         group_elem_parent_term,inlist,point_internal_to_surface, &
         reallocate_node_elem_arrays,triangles_from_surface, &
    ...
    logical :: make_branch,enough_points(2),first_group,internal, &
         ...
         limit_branching_plane = .false.    ! option to restrict angle between branching planes (not working?)
    ...
    allocate(parentlist(num_data))
    call group_elem_parent_term(parent_ne,parentlist)
